...
type ConnectionState = "checking" | "connecting" | "loading" | "connected" | "error";
...
function txKindLabel(kind: string): string {
  switch (kind) {
    ...
    case "commitment": return "Round";
    case "ark": return "Ark Transfer";
...
export function DashboardRoute() {
  ...
  const fetchData = useCallback(async (initial?: boolean) => {
    ...
    try {
      ...
      if (cancelledRef.current || id !== fetchIdRef.current) return;
      const message = typeof error === "string" ? error : "Failed to fetch wallet data";
      if (initial) {
  ...
  useLayoutEffect(() => { fetchRef.current = fetchData; }, [fetchData]);
  ...
  const totalSat = (balance?.onchain_confirmed_sat ?? 0) + (balance?.offchain_total_sat ?? 0);

  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white" style={{ paddingTop: "env(safe-area-inset-top, 0px)" }}>
      {/* Header */}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              autoRefresh ? "bg-lime-300/20 text-lime-300" : "bg-white/10 text-white/40"
            }`}
      ...
      <div className="px-6 pt-4 pb-6 text-center">
        ...
        <p className="text-4xl font-bold tabular-nums">{formatSats(totalSat)} <span className="text-lg text-white/50">sats</span></p>
      </div>
      ...
      <div className="mx-6 grid grid-cols-2 gap-3 mb-6">
        <div className="rounded-2xl bg-white/5 p-4">
          ...
          <p className="text-lg font-semibold tabular-nums">{formatSats(balance?.onchain_confirmed_sat ?? 0)}</p>
          {(balance?.onchain_pending_sat ?? 0) > 0 && (
            <p className="text-xs text-yellow-300/70 mt-0.5">+{formatSats(balance!.onchain_pending_sat)} pending</p>
          )}
        </div>
        <div className="rounded-2xl bg-white/5 p-4">
          <p className="text-xs text-white/40 mb-1">Offchain (Ark)</p>
          <p className="text-lg font-semibold tabular-nums">{formatSats(balance?.offchain_total_sat ?? 0)}</p>
          {(balance?.offchain_pre_confirmed_sat ?? 0) > 0 && (
            <p className="text-xs text-yellow-300/70 mt-0.5">{formatSats(balance!.offchain_pre_confirmed_sat)} pre-confirmed</p>
          )}
        </div>
      </div>
      ...
      {/* Transactions */}
      <div className="px-6">
        <h2 className="text-sm font-semibold text-white/50 mb-3">Recent Transactions</h2>
        {transactions.length === 0 ? (
          <div className="rounded-2xl bg-white/5 p-8 text-center">
            <p className="text-white/40 text-sm">No transactions yet</p>
          </div>
        ) : (
          <div className="space-y-2" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 24px)" }}>
            {transactions.map((tx, i) => (
              <div key={`${tx.txid}-${tx.kind}-${i}`} className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium">{txKindLabel(tx.kind)}</span>
                    {tx.is_settled === false && (
                      <span className="rounded-full bg-yellow-300/20 px-2 py-0.5 text-[10px] font-medium text-yellow-300">Pending</span>
                    )}
                  </div>
                  <p className="text-xs text-white/40 mt-0.5">{formatDate(tx.created_at)}</p>
                </div>
                <p className={`text-sm font-semibold tabular-nums ${tx.amount_sat >= 0 ? "text-lime-300" : "text-red-300"}`}>
                  {tx.amount_sat >= 0 ? "+" : ""}{formatSats(tx.amount_sat)} <span className="text-[10px] text-white/30">sats</span>
                </p>
              </div>
            ))}
          </div>
